...
MapReads() {
	...
	bash map_reads_unmerged.sh -i $wd/trimmed_reads/unmerged_reads -g $wd/references/${2} -o $wd/mapped_reads/${1} -t 2
}
export -f MapReads
...
grep -v '^#' ref_genomes_2.txt | parallel --colsep ' ' MapReads {1} {2}
echo "mapping done"
